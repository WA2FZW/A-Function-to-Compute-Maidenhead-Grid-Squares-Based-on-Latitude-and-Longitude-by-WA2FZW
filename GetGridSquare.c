...
char	upperCase[] = "ABCDEFGHIJKLMNOPQR";			// Letters for first 2 characters
char	lowerCase[] = "abcdefghijklmnopqrstuvwx";	// Letters for the last 2 characters
 ...
 */
		...
		buff[0] = upperCase[index];					// Set first character
		...
		index = (int) tempNumber;					// The index to the "upperCase" array
		buff[1] = upperCase[index];					// Set second character
 ...
 */
		...
		buff[2] = digits[index];					// Set third character
		_long = _long - ( index * 2 );				// Remainder for step 3

		tempNumber = _lat;							// Remainder from step 1 divided by 1
		index = (int) tempNumber;					// The index to the "digits" array
		buff[3] = digits[index];					// Set fourth character
		_long = _long - index;						// Remainder for step 3
 ...
 */
		...
		buff[4] = lowerCase[index];					// Set fifth character

		tempNumber = _lat / 0.0416665;				// Remainder from step 2 divided by 0.0416665
		index = (int) tempNumber;					// The index to the "digits" array
		buff[5] = lowerCase[index];					// Set sixth character
